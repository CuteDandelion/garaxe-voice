...
The current MVP implements the responsive editorial shell, persistent CSV/XLSX/JSON/paste ingestion and review inventory, immutable deterministic analysis runs, exact-span signals, validated themes, Read/Investigate Voice Maps, append-only human curation with publication gates, immutable report versions, evidence-backed PDF export, organization-scoped sessions, and a managed-PostgreSQL adapter. Google OAuth, account/location discovery, explicit location selection, complete review sync, refresh/revoke, and access diagnostics are implemented and contract-tested; provider approval and live-profile proof remain external readiness gates.
...
The current analysis pipeline is deterministic and model-free: immutable runs preserve dataset membership, exact evidence spans, validated themes, and a versioned machine Voice Map. Run-scoped curation records approve, reject, edit, pin, exclude, merge, split, and mark-ready decisions as an append-only audit stream. Ready Read/Investigate views use the human-approved projection while retaining the machine artifact underneath.

Next: apply the locally rehearsed forced-RLS migration to a live managed database with a least-privilege runtime role and KMS-backed key rotation, then prove the implemented Google OAuth/sync flow against an approved project plus a real verified managed profile.
