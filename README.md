...
The Bluerose overlay also enables the evaluated OpenCode Go `qwen3.7-plus` cluster interpreter with a required server-side Secret, explicit request/token/concurrency limits, and a documented public-HTTPS staging egress exception. A fresh live run completed three provider jobs without fallback and published two validated model-backed themes; the bounded proof is in `docs/deployment-evidence/2026-07-14-bluerose-llm.md`. Deterministic analysis remains the immutable evidence authority and is labeled as degraded fallback whenever validated model interpretation is unavailable.
