...
- Static: formatting, lint, strict typecheck, dependency/security checks.
...
Every change must pass docs sync, static checks, unit tests, relevant integration tests, and build. Merge-blocking E2E covers the critical happy path; scheduled tests cover external provider smoke tests where credentials and provider policies permit.
...
- UI tests cover sample import, actual browser `File` input, project creation and switching, authenticated identity, server-backed logout, evidence drawer, tab selection, and JSON export.
...
- `npm run evaluate:semantic` runs the pinned production embedding model against the pattern-diverse semantic gold set and fails on purity, recall, coverage, cross-topic-merge, or mixed-cluster-adjudication regressions.
...
- Managed PostgreSQL adapter tests prove parameter forwarding, pool queries, transaction commit/rollback, and client release without requiring cloud credentials in CI.
- Migration tests apply the forced-RLS schema idempotently, verify all 18 tenant-owned tables and policies, and use a least-privilege role to prove transaction-scoped tenant filtering plus same-tenant project creation.
...
- Automated axe scans cover the authenticated Overview and Sources structures; color contrast remains a rendered/manual audit because JSDOM cannot calculate the production palette.
